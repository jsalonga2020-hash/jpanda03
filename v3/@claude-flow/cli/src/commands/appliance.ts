...
const buildCommand: Command = {
  name: 'build',
  description: 'Build a self-contained ezra.rvf appliance',
  options: [
    { name: 'profile', short: 'p', type: 'string', description: 'Build profile: cloud, hybrid, offline', default: 'cloud' },
    { name: 'output', short: 'o', type: 'string', description: 'Output file path', default: 'ezra.rvf' },
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    const outputPath = ctx.flags.output as string || 'ezra.rvf';
    ...
    const steps = [
      ...
      'Packaging ezra CLI + MCP tools', 'Compressing sections',
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      for (const [label, value] of [
        ['Name', hdr.name || 'ezra'], ['Version', hdr.version || 'unknown'],
...
const extractCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      for (const d of ['kernel', 'runtime', 'ezra', 'models', 'data', 'verify']) {
...
export const applianceCommand: Command = {
  ...
  examples: [
    { command: 'ezra appliance build -p cloud', description: 'Build a cloud appliance' },
    { command: 'ezra appliance inspect -f ezra.rvf', description: 'Inspect appliance contents' },
    { command: 'ezra appliance verify -f ezra.rvf', description: 'Verify integrity' },
    { command: 'ezra appliance extract -f ezra.rvf', description: 'Extract sections' },
    { command: 'ezra appliance run -f ezra.rvf', description: 'Boot and run appliance' },
    { command: 'ezra appliance sign -f ezra.rvf --generate-keys', description: 'Generate keys and sign' },
    { command: 'ezra appliance publish -f ezra.rvf', description: 'Publish to IPFS via Pinata' },
    { command: 'ezra appliance update -f ezra.rvf -s ezra -d ./new-ezra.bin', description: 'Hot-patch a section' },
  ],
  action: async (): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('Ezra Appliance (RVFA)'));
    output.writeln(output.dim('Self-contained deployment format for the full Ezra platform.'));
    output.writeln();
    output.writeln('Subcommands:');
    output.printList([
      'build     - Build a self-contained ezra.rvf appliance',
    ...
    output.writeln();
    output.writeln(output.dim('Use "ezra appliance <subcommand> --help" for details.'));
    return { success: true };
  },
};
