...
const startAction = async (ctx: CommandContext): Promise<CommandResult> => {
  ...
  if (!isInitialized(cwd)) {
    output.printError('Ezra is not initialized in this directory');
    output.printInfo('Run "ezra init" first to initialize');
  ...
  output.writeln();
  output.writeln(output.bold('Starting Ezra'));
  output.writeln();
  ...
  try {
    ...
    // Success output
    output.writeln();
    output.printSuccess('Ezra is running!');
    output.writeln();
...
const stopCommand: Command = {
  name: 'stop',
  description: 'Stop the Ezra system',
  options: [
    {
      name: 'force',
      short: 'f',
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln(output.bold('Stopping Ezra'));
    output.writeln();

    if (!force && ctx.interactive) {
      const confirmed = await confirm({
        message: 'Are you sure you want to stop Ezra?',
        default: false
      });
    ...
    try {
      ...
      output.writeln();
      output.printSuccess('Ezra stopped successfully');
...
const restartCommand: Command = {
  name: 'restart',
  description: 'Restart the Ezra system',
  options: [
    {
      name: 'force',
      short: 'f',
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('Restarting Ezra'));
    output.writeln();
...
export const startCommand: Command = {
  name: 'start',
  description: 'Start the Ezra orchestration system',
