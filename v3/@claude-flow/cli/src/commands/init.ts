...
async function initCodexAction(
  ...
  output.writeln();
  output.writeln(output.bold('Initializing Ezra for OpenAI Codex'));
  output.writeln();
...
const initAction = async (ctx: CommandContext): Promise<CommandResult> => {
  ...
  if (hasExisting && !force) {
    output.printWarning('Ezra appears to be already initialized');
  ...
  output.writeln();
  output.writeln(output.bold('Initializing Ezra'));
  output.writeln();
  ...
  try {
    ...
    spinner.succeed('Ezra initialized successfully!');
    output.writeln();
...
const wizardCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('Ezra Setup Wizard'));
...
const checkCommand: Command = {
  name: 'check',
  description: 'Check if Ezra is initialized',
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (result.initialized) {
      output.printSuccess('Ezra is initialized');
      ...
      output.printWarning('Ezra is not initialized in this directory');
      output.printInfo('Run "ezra init" to initialize');
    }
...
const upgradeCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln(output.bold('Upgrading Ezra'));
...
export const initCommand: Command = {
  name: 'init',
  description: 'Initialize Ezra in the current directory',
